...
public class Pattern {
	...
	public Pattern(String format) throws BadFormatException {
		//NAME:AUTHOR:WIDTH:HEIGHT:STARTCOL:STARTROW:CELLS
		String[] formatStrings = format.split(":");
		if (formatStrings.length!=7) throw new BadFormatException("Incorrect number of arguments");
		name = formatStrings[0];
		author = formatStrings[1];
		cells = formatStrings[6];
		for (char i:cells.toCharArray()) {
			if (i!='0' & i!='1' & i!=' ') throw new BadFormatException("Cells contain illegal values");
		}
		try{
			...
			throw new BadFormatException("Unable to convert string to value "+e.getMessage());
		}
